...
| Components | Buttons with emoji/link/premium styles, select menus with option emoji, text inputs, action rows, modals, single-item helpers, validation, Components V2 layout components, Discord.js-style component enum aliases, and builder aliases |
...
| Intents, partials, permissions, and channel types | Current bit constants and channel type values, Discord.js-compatible aliases, aggregate helpers, overwrite resolution, permission sets, partial masks, and name mapping |
| Utilities | Formatters, timestamp/hide-link helpers, targeted markdown escape helpers, mention aliases, guild-template/webhook links, OAuth authorization URLs, expanded asset/CDN URL helpers, Discord.js color palette aliases, snowflakes, and JSON escaping |
